...
class FixWallGran : public Fix, public LIGGGHTS::IContactHistorySetup {
 ...
 protected:
  ...
  int nPrimitiveArgs; //~ Number of primitive wall agruments: 11 for ycylinder_filtermesh
  int np_ver, np_hor; //~ Defined in primitive_wall_definitions.h
  double ylow, yhigh; //~ Defined in primitive_wall_definitions.h
  double fvoid_ver, fvoid_hor, ftransition_ver, ftransition_hor; //~ Defined in primitive_wall_definitions.h
  double C_x, C_y, C_z; //~ Coordinate of the rectangular y-plane (left bottom)
  double A_ver, A_hor; //~ Defined in the primitive_wall_definitions.h
  double shape; //~ Defined in the primitive wall definitions.h '0' dummy variable
  double fsolid_ver, fsolid_hor; //~ Fractional distances which are solid in the repeating cells
  double rcylinder; //~ Radius of the cylindrical basket
  double Lv, Lh; //~ Defined in the constructor
  double fdv, fdh; //~ Used in the calculation of forces applied in the transition regions
  double criticalradius; //~ The particle radius above which the particle can never even come into contact with the void and transition regions
  double direc;
  double rtop;
  double rcircle;
  double rocircle;
  int outside_basket_check(double, double, double, int);
  int outside_rectangle_check(double,double,int);
  int region_finder(double, double, double, double);
};
